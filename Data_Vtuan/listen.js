...
(async function () {
    ...
    const data_anti =
            JSON.parse(
                fs.readFileSync(
                    process.cwd() + "/modules/commands/src/anti_data.json",
                    "utf8"
                )
            ) || [];
        const { avtbox } = data_anti.find(
    ...
    api.unsendMessage(event.messageID)
        ...
        const ty =
                    api.getCurrentUserID() === event.author
                        ? false
                        : !dataThread.adminIDs.some(
                              (item) => String(item.id) === String(event.author)
                          );
                if (
                    avtbox.status && ty
                ) {
                    dataThread["imageSrc"] = avtbox.url;
                    api.sendMessage(
                        `» [ ANTI AVT BOX ]\n» Đã Kick hoạt cấm đổi avt box`,
                        event.threadID,
                        async (error, info) => {
                            const path =
                                process.cwd() +
                                `/modules/commands/src/change_thread_image${event.threadID}.png`;
                            let { data: getImg } = await axios.get(avtbox.url, {
                    ...
                    dataThread["imageSrc"] = res.data.link;
                    const path =
                        process.cwd() +
                        `/modules/commands/src/change_thread_image${event.threadID}.png`;
                    let { data: getImg } = await axios.get(res.data.link, {
                        responseType: "arraybuffer",
                    });
                    fs.writeFileSync(path, Buffer.from(getImg, "utf-8"));
                    api.sendMessage(
                        {
                            body: `[ UPDATE GROUP ]\n• Người dùng: ${event.author}\n• Đã thay đổi ảnh nhóm`,
                ...
                return fs.writeFileSync(
                    process.cwd() + "/modules/commands/src/anti_data.json",
                    JSON.stringify(data_anti, null, 4)
                );
                break;
			default:
				break;
